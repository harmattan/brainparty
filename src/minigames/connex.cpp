...
BPMiniGame_Connex::BPMiniGame_Connex(BPGame* game) : BPMiniGame(game) {
	...
	if (!sfcBackground) {
		throw std::runtime_error("Failed to load background bitmap.");
	}

	sfcCard = TheGame->LoadBitmap("card_blank", 64, 64);
	if (!sfcCard) {
		throw std::runtime_error("Failed to load card bitmap.");
	}
...
void BPMiniGame_Connex::Render() {
	...
	for (int i = 0; i < Objects->Count; ++i) {
		...
		if (LastLetter != -1 && (*Letters)[LastLetter] == obj->Text) {
	...
	for (int i = 0; i < CompleteLetters; ++i) {
		BPMiniGame_Connex_Object* from = nullptr;
		BPMiniGame_Connex_Object* to = nullptr;
